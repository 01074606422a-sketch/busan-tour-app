...
/* [핵심] 페르소나 테스트 질문-항목 열 맞춤 및 간격 교정 */
[data-testid="stForm"] .stRadio {
    display: flex;
    justify-content: center; /* 라디오 버튼 블록 전체를 가운데로 정렬 */
    width: 100%;
}
[data-testid="stForm"] .stRadio > div[role="radiogroup"] {
    width: fit-content !important; /* 텍스트 길이만큼만 박스 크기 지정 */
    margin: 0 auto;
    background: transparent !important;
    padding: 0 !important;
    display: flex;
    flex-direction: column;
    align-items: flex-start; /* 텍스트 시작점은 왼쪽으로 통일 */
}
/* 각 항목(선택지) 사이의 위아래 간격 넉넉하게 띄우기 */
...
if not st.session_state.test_completed:
    ...
    with st.form("persona_test"):
        ...
        for i, q_data in enumerate(QUESTIONS):
            ...
            # [핵심] 컬럼 분리 코드를 제거하여 라디오 버튼 그룹이 화면 중앙에서 온전히 센터 정렬되도록 함
            ...
            st.markdown("<hr style='border: 1px dashed #BAE6FD; margin: 30px 0;'>", unsafe_allow_html=True)
