...
# 1. 환경 설정 및 세션 상태
# --------------------------------------------------------------------------
st.set_page_config(page_title="부산 시티투어 가이드", page_icon="🌊", layout="wide")

custom_css = """
<style>
/* 가장 세련되고 덜 정적인 고딕체 웹폰트 (Pretendard) 로드 */
@import url('https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/static/pretendard.css');

/* 시원한 부산 바다 파도 느낌의 깔끔한 배경 테마 */
.stApp {
    background: radial-gradient(circle at 50% -20%, #E0F7FA 0%, #F0F9FF 30%, #E0F2FE 70%, #BAE6FD 100%) !important;
    background-attachment: fixed !important;
    font-family: 'Pretendard', sans-serif !important;
}

/* 양옆 여백 최소화 (모바일/PC 모두 최적화) */
.block-container {
...
/* =========================================================
   [페르소나 폼 전용] 질문지 라디오 버튼 완벽 중앙 정렬 및 반응형
   ========================================================= */
...
[data-testid="stForm"] div[role="radiogroup"] label p {
...
/* =========================================================
   [메뉴 탭 전용] 체크리스트를 앱 스타일 '버튼(탭)'으로 완벽 변신!
   ========================================================= */
div[data-testid="stRadio"]:not([data-testid="stForm"] div[data-testid="stRadio"]) > div[role="radiogroup"] {
    display: flex !important;
...
div[data-testid="stRadio"]:not([data-testid="stForm"] div[data-testid="stRadio"]) > div[role="radiogroup"] label > div:first-child {
    display: none !important;
}
div[data-testid="stRadio"]:not([data-testid="stForm"] div[data-testid="stRadio"]) > div[role="radiogroup"] label {
...
div[data-testid="stRadio"]:not([data-testid="stForm"] div[data-testid="stRadio"]) > div[role="radiogroup"] label:hover {
    border-color: #0078D7 !important;
    transform: translateY(-3px) !important;
    box-shadow: 0 6px 12px rgba(0, 198, 255, 0.2) !important;
}
div[data-testid="stRadio"]:not([data-testid="stForm"] div[data-testid="stRadio"]) > div[role="radiogroup"] label p {
...
div[data-testid="stRadio"]:not([data-testid="stForm"] div[data-testid="stRadio"]) > div[role="radiogroup"] label:has(input:checked) {
    background: linear-gradient(135deg, #0078D7 0%, #00C6FF 100%) !important;
    border-color: transparent !important;
    box-shadow: 0 6px 15px rgba(0, 198, 255, 0.4) !important;
}
div[data-testid="stRadio"]:not([data-testid="stForm"] div[data-testid="stRadio"]) > div[role="radiogroup"] label:has(input:checked) p {
...
def generate_congestion_trend(stop_name, is_weekend):
    ...
    base_trend = []
    seed_val = sum(ord(c) for c in stop_name) + (100 if is_weekend else 0)
...
def load_attraction_data():
    ...
    try:
        ...
        df_excel = df_kr[df_kr['tourist_tycd'] == 'CD0000001275'][["MAIN_TITLE", "LAT", "LNG", "ITEMCNTNTS", "MAIN_IMG_NORMAL", "ADDR1", "USAGE_DAY_WEEK_AND_TIME", "TRFC_INFO"]]
    except:
    ...
    try:
        ...
        df_csv = df_csv[["MAIN_TITLE", "LAT", "LNG", "ITEMCNTNTS", "MAIN_IMG_NORMAL", "ADDR1", "USAGE_DAY_WEEK_AND_TIME", "TRFC_INFO"]]
    except:
        df_csv = pd.DataFrame()
...
if not st.session_state.test_completed:
    ...
    st.markdown("<p class='center-text' style='font-size: 19px; color: #333; font-family: \"Pretendard\", sans-serif; margin-bottom: 30px;'>본격적인 시티투어 전, <b>10가지 질문</b>으로 당신의 진짜 여행 취향을 분석해 드립니다!</p>", unsafe_allow_html=True)
    
    with st.form("persona_test"):
        responses = []
        for i, q_data in enumerate(QUESTIONS):
            st.markdown(f"<h3 class='center-text' style='color: #004E98; margin-bottom: 15px;'>{q_data['q']}</h3>", unsafe_allow_html=True)
        ...
        with col_btn2:
            if st.form_submit_button("부산 여행 시작하기", use_container_width=True):
...
# --------------------------------------------------------------------------
    ...
    # ------------------ 화면: 시공간 혼잡도 예측 ------------------
        ...
        st.markdown(f"### 📊 `{selected_stop}` 혼잡도 예측")
        ...
        base = alt.Chart(chart_df).encode(x=alt.X('시간:O', title='시간대', axis=alt.Axis(labelAngle=0, labelOverlap=True)))
                ...
                with st.chat_message("assistant"):
                    ...
                    def fetch_gemini():
                        try:
                            ...
                            system_prompt = f"""
                            당신은 부산 여행객에게 오직 '실제로 존재하는' 식당과 명소만 안내하는 AI 가이드입니다.
                            [🚨 할루시네이션(거짓 정보 생성) 절대 금지 규칙]
                            1. 질문자가 맛집을 추천해달라고 하면, '네이버 지도'나 '카카오맵'에 검색했을 때 무조건 나오는 유명하고 대중적인 식당(또는 확실한 노포) 위주로 추천하세요.
                            2. 멋진 뷰나 대단한 분위기보다 '실존 여부'와 '정확도'가 무조건 1순위입니다. 이름이 헷갈리거나 불확실한 곳은 절대 지어내지 마세요.
                            3. 만약 질문 조건에 딱 맞는 식당을 확실히 모른다면, 차라리 근처의 가장 유명하고 검증된 프랜차이즈나 유명 대형 식당을 대안으로 추천하세요.
                            4. 추천할 때는 [상호명]과 [주요 메뉴]를 명확히 적어주세요.
                            5. 답변은 친절하고 이모지를 섞어서 가독성 좋게 작성하세요.
