                ...
                for idx, row in final_themes.iterrows():
                    with st.container():
                        ...
                        with col_img:
                            ...
                            # [핵심] 이미지가 예쁘게 비율대로 잘려 들어가도록 object-fit: cover 적용 및 여백 박스 제거
        ...
        if target_attrs.empty:
            ...
            for i, (idx, row) in enumerate(target_attrs.iterrows()):
                ...
                with col_l:
                    ...
                    # [핵심] 명소 가이드에서도 이미지가 꽉 차고 여백이 없도록 object-fit: cover 적용
            ...
            for i, (idx, row) in enumerate(final_foods.iterrows()):
                with cols[i % 2]:
                    with st.container():
                        ...
                        # [핵심] 식당 사진도 예쁘게 꽉 차도록 object-fit: cover 적용
                ...
                with st.chat_message("assistant"):
                    ...
                    def fetch_gemini():
                        try:
                            ...
                            # [핵심] 없는 모델을 지우고 가장 지능이 뛰어난 gemini-1.5-pro 모델로 교체!
                            model = genai.GenerativeModel('gemini-3.7-flash')
                            
                            # [핵심] 할루시네이션(거짓 정보) 방지를 위해 시스템 프롬프트 강력하게 수정
                            system_prompt = f"""
                            당신은 부산 전문 로컬 투어 가이드입니다. 사용자에게 최고의 경험을 제공해야 합니다.
                            [🚨 절대 준수 사항: 할루시네이션(거짓 정보) 방지]
                            1. 추천할 때는 반드시 지도에 검색하면 나오는 100% 실제 존재하는 유명 상호명만 언급하세요.
                            2. 100% 확신하는 곳이 아니라면 절대 지어내지 말고, "죄송하지만 해당 조건에 딱 맞는 검증된 곳은 제 데이터에 없습니다."라고 대답하세요.
