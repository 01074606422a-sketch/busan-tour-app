...
/* [핵심] 페르소나 폼 내부의 라디오버튼만 중앙으로 모으고 줄 간격 맞춤 */
...
[data-testid="stForm"] div[role="radiogroup"] {
    ...
    gap: 14px !important; /* 상하 항목 줄 간격을 딱 맞게 조정 */
    width: fit-content !important; 
    margin: 0 auto !important; /* 중앙 정렬의 핵심 */
    background: transparent !important;
    padding: 0 !important;
}
[data-testid="stForm"] div[role="radiogroup"] label {
    margin-bottom: 0 !important; /* 기존 여백 무시하고 gap에 의존 */
}
...
if not st.session_state.test_completed:
    ...
    st.markdown("<p class='center-text' style='font-size: 18px; color: #333; font-family: \"Pretendard\", sans-serif;'>본격적인 시티투어 전, <b>10가지 질문</b>으로 당신의 진짜 여행 취향을 분석해 드립니다!</p><br>", unsafe_allow_html=True)
    
    with st.form("persona_test"):
        responses = []
        for i, q_data in enumerate(QUESTIONS):
            st.markdown(f"<h4 class='center-text' style='color: #0078D7; margin-bottom: 25px;'>{q_data['q']}</h4>", unsafe_allow_html=True)
            
            # [핵심] 양옆에 빈 기둥을 세워 항목 박스를 화면 정중앙으로 밀어버립니다
            ...
            st.markdown("<hr style='border: 1px dashed #BAE6FD; margin: 40px 0;'>", unsafe_allow_html=True)
